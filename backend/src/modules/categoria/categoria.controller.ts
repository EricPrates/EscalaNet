...
import { SchemaFiltrosCategoria, SchemaCriarCategoria, SchemaAtualizarCategoria, SchemaBuscarPorIdCategoria,  QueryIncludesCategoria, SchemaBuscarPorNomeCategoria } from './categoria.schemas';
...
export function fazerCategoriaController(service: ICategoriaService) {
    return {
        ...
        async obterCategoriaPorNome(req: Request, res: Response) {
            const {nome} = SchemaBuscarPorNomeCategoria.parse(req.params);
            const categoria = await service.obterPorNome(nome);
            return res.status(200).json(montarRespostaSucesso('Categoria obtida com sucesso', categoria));
        },
